...
def log_additions(additions, l1reg, l2reg, augmentation, dropout=None, weight_decay=None, crop_flipped_only=False):
...
def all_log_dir_args(
    ...
    dir_name = "{}/{}/{}/{}".format(
        ...
        model,
        data,
        log_additions(model_additions, l1reg, l2reg, augmentation, dropout, weight_decay, crop_flipped_only),
    )
    sub_dir_name = ""
    if label_noise != 0:
        sub_dir_name = "noise-{:.4f}".format(label_noise)
    if noise_type != "symmetric":
        sub_dir_name = "asymmetric_" + sub_dir_name
    if seed is not None:
...
def checkpoint_dir_args(
    ...
    return "{}/checkpoints".format(
        all_log_dir_args(
            base, model, data, label_noise, seed, l1reg, l2reg, name, augmentation, model_additions, dropout, weight_decay, crop_flipped_only, noise_type
        )
    )
